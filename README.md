...
- **Letters**: the **Letters** page turns pasted text into a formatted business letter on your firm's letterhead — a centered logo at the top and an address / phone / fax bar at the bottom, matching the firm's standard letter format (centered bold date, bold delivery line, `Re:` line, block-paragraph body in Calibri, centered signature block, and `xc:` / Enclosure notations). Set the logo, footer address, phone/fax, and default signature once under **Edit letterhead** (stored in the database — the firm's details are pre-filled by default). For each letter, fill in the date, delivery line, recipient, `Re:` line, and salutation, then paste the body (paragraphs separated by blank lines). You can prefill the recipient from any existing client. Click **Print / Save as PDF** and use your browser's print dialog; only the letter sheet prints, not the app chrome. (If you're upgrading an existing database, run `npm run db:push` once to add the letterhead-settings table.)
